							 ...
							 struct ndayflux_patch_struct *ndf)
{
	...
	// This equation is a modification of their eqn. 11, changing the base temperature from 10 C to 25 C, since most of
	// the microcosm studies used to get the base decomp rates were controlled at 25 C. 
	//=====================================================================================================================
	if (tsoil < -10.0){
	...
	//=====================================================================================================================
	// calculate the rate constant scalar for soil water content. use same empirical function as control on nitrification
	// from NGas model but set parameters to reduce sensitivity to water stress
	...
	if (std > ZERO) {
		theta1 = 1.0;
		theta2 = 0.3;
		water_scalar1  = pow( ((theta1 -b) / (a-b)), d*(b-a)/(a-c))	* pow( ((theta1-c)/ (a-c)), d);
		water_scalar2  = pow( ((theta2 -b) / (a-b)), d*(b-a)/(a-c))	* pow( ((theta2-c)/ (a-c)), d);
		...
		if (theta  > c)
			w_scalar = pow( ((theta -b) / (a-b)), d*(b-a)/(a-c)) * pow( ((theta-c)/ (a-c)), d);
		else
			w_scalar = 0.000001;
	}
	...
	//=====================================================================================================================
	// calculate the non-nitrogen limited fluxes between litter and soil compartments. These will be ammended for 
	// N limitation if it turns out the potential gross immobilization is greater than potential gross mineralization. 
	//=====================================================================================================================
	...
	//=====================================================================================================================
	// determine if there is sufficient mineral N to support potential immobilization. 
	// Immobilization fluxes are positive, mineralization fluxes are negative 
	//=====================================================================================================================
	nlimit          = 0;
	potential_immob = 0.0;
	mineralized     = 0.0;
	...
	//=====================================================================================================================
	// save the potential fluxes until plant demand has been assessed, to allow competition between immobilization fluxes
	// and plant growth demands 
	//=====================================================================================================================
