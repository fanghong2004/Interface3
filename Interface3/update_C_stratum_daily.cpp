						   ...
						   struct cstate_struct *cs, struct cdayflux_struct *cdf)
{
	...
	if((cdf->psn_to_cpool - (cdf->total_mr + cdf->total_gr))<0) {
		//printf("cdf_psn is %f %f %f \n", cdf->psn_to_cpool, cdf->total_mr, cdf->total_gr);
		//getchar();
		cdf->total_mr=0.125*cdf->psn_to_cpool;
		cdf->total_gr=0.375*cdf->psn_to_cpool;
	}

	cdf->net_psn = cdf->psn_to_cpool - cdf->total_mr - cdf->total_gr;
	//printf("psn_to_cpool is %12.8f %12.8f %12.8f \n",cdf->potential_psn_to_cpool,cdf->total_mr,cdf->total_gr);

	// Daily allocation fluxes 
	// daily leaf allocation fluxes 
	//printf("cdf->cpool_to_leafc %f  \n",cdf->cpool_to_leafc);
	...
	// Daily growth respiration fluxes Leaf growth respiration 
