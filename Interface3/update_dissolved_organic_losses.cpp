...
//	PROGRAMMER NOTES					
//								
//	assume DON at 3% mineralization (Vitousek et al,2000; Within-system element cycles, Input-output budgets      
				  ...
				  struct  ndayflux_patch_struct *ndf)
{
	...
	if (ns_litr->litr3n > ZERO) 
		cn_l3 = cs_litr->litr3c/ns_litr->litr3n;
	else 
		cn_l3 = CEL_CN;
	...
	// shielded cellulose litter fluxes note these are based on lignan decay 
	...
	cs_litr->litr3c       -= cdf->do_litr3c_loss; //added by guoping???
	...
	ns_litr->litr3n       -= ndf->do_litr3n_loss; //added by guoping???
	...
	cdf->total_DOC_loss =  (cdf->do_litr1c_loss + cdf->do_litr2c_loss + cdf->do_litr3c_loss +cdf->do_litr4c_loss + 
				            cdf->do_soil1c_loss + cdf->do_soil2c_loss + cdf->do_soil3c_loss +cdf->do_soil4c_loss);
	                        //Note::cdf->do_litr3c_loss and cdf->do_soil3c_loss were added by guoping

	ndf->total_DON_loss =  (ndf->do_litr1n_loss + ndf->do_litr2n_loss + ndf->do_litr4n_loss +ndf->do_litr3n_loss + 
				            ndf->do_soil1n_loss + ndf->do_soil2n_loss + ndf->do_soil4n_loss +ndf->do_soil3n_loss);
	                        //Note::ndf->do_soil3n_loss wad added by guoping
