						  ...
						  struct date current_date)
{
	...
	double  add_to_cpool; //closs, nloss,
	double plant_remaining_ndemand;
	double excess_allocation_to_leaf, excess_c,  excess_lai; //excess_n,
	...
	//===============================================================================================================================
	f1   = epc.alloc_frootc_leafc;      // (ratio) new fine root C to new leaf C       
	f2   = epc.alloc_crootc_stemc;		// (ratio) new live croot C to new live stem C 
	f3   = epc.alloc_stemc_leafc;		// (ratio) new live stem C to new leaf C       
	f4   = epc.alloc_livewoodc_woodc;	// (ratio) new live wood to total wood C       
	g1   = epc.gr_perc;
	cnl  = epc.leaf_cn;
	cnfr = epc.froot_cn;
	...
	//===============================================================================================================================
	// the amount available from the soil is potential_N_uptake adjusted by fract_potential_uptake which is calculated 
	// in resolve_N_competition based on available soil mineralized nitrogen if there are no roots then we uptake anyway 
	// to get things started 
	//===============================================================================================================================
	//nlimit =0; //double check this ???
	
	//if(nlimit==0) {
	//	printf("nlimit is %12.8f \n",nlimit);
	//	getchar();
	//}
	if (nlimit == 1)
	...
	//===============================================================================================================================
	//	now compare the combined decomposition immobilization and plant growth N demands against the available soil mineral N pool.
	//===============================================================================================================================
	if (nlimit == 0){
		//===========================================================================================================================
	    // N availability is not limiting so plant uptake, and both can proceed at  potential rates
		...
		//===============================================================================================================================
	    // N availability can not satisfy the sum of immobiliation and plant growth demands, so these two demands compete for available
		// soil mineral N
		//===============================================================================================================================
		...
		// back to a C excess, which is deducted from photosynthesis source
	...
	//===============================================================================================================================
	// calculate the amount of new leaf C dictated by these allocation decisions, and figure the daily fluxes of C and N to current
	// growth and storage pools given the available C, use constant allometric relationships to
	// determine how much N is required to meet this potential growth demand
	//===============================================================================================================================
	if (epc.veg_type == TREE) 	{
		c_newleafc = cs->availc / (1.0+g1+f1+f1*g1+f3+f3*g1+f2*f3+f2*f3*g1);
		n_newleafc = plant_nalloc / (1.0/cnl + f1/cnfr + (f3 * f4 * (1.0+f2))/cnlw + (f3 * (1.0-f4) * (1.0+f2))/cndw);
	 }
	 else {
	...
	//if (current_date.day < 220){
		cdf->cpool_to_leafc = nlc * pnow;
		cdf->cpool_to_leafc_store = nlc * (1.0 - pnow);
	//}
	//else{
	//	cdf->cpool_to_leafc = 0;
	//	cdf->cpool_to_leafc_store = nlc ;
	//}
