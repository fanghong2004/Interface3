											 ...
											 
{
	...
	if (LWP_predawn !=  9999.0) {
		m_LWP = leaf_conductance_LWP_curve(LWP_predawn,	LWP_min_spring,LWP_stom_closure);
		/*
		if ((patch->rz_storage > ZERO) && (patch->sat_deficit > stratum->rootzone.potential_sat)) {
			wilting_point = exp(-1.0*log(-1.0*LWP_stom_closure/patch->soil_defaults[0]->psi_air_entry) 
			* patch->soil_defaults[0]->pore_size_index) * patch->soil_defaults[0]->porosity_0;
			if (patch->rz_storage/(min(patch->sat_deficit, patch->rootzone.potential_sat)) 
	...
	/*
	stratum->mult_conductance.APAR = m_APAR;			
	stratum->mult_conductance.tavg = m_tavg;			
	stratum->mult_conductance.LWP = m_LWP;			
	stratum->mult_conductance.CO2 = m_CO2;			
	stratum->mult_conductance.tmin = m_tmin;			
	stratum->mult_conductance.vpd = m_vpd;			
	*/
