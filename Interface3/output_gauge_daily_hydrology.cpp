	...
	FILE *outfile, struct command_line_object *command_line,int cellsize)
{
	...
	//xu. m^3/day
	fprintf(outfile, "%12.3f", (astreamflow)*pow(cellsize, 2) / seconds_per_day); //+ gw_discharge
	fprintf(outfile, "%12.3f", (abase_flow) *pow(cellsize, 2) / seconds_per_day); //+ gw_discharge
	fprintf(outfile, "%12.3f", areturn_flow *pow(cellsize, 2) / seconds_per_day);
