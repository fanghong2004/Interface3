...
*/
	h_o=min(max(h_o,0.010),z/0.9); //added by guoping to avoid zero screen height
...
*/
	...
	/*--------------------------------------------------------------*/
	//the following is modified by guoping to avoid 0 m/s wind speed. otherwishe
	// because *u can not be zero in calculating ra_u (see below)
	//*u = max((u_o * exp(cn * (max(h_u, 0.1*h_o)/h_o - 1))), 0.); //original
	*u = max((u_o * exp(cn * (max(h_u, 0.1*h_o)/h_o - 1))), 0.1);  //modified by guoping
