							...
							struct cstate_struct *cs)
{
	...
	if ( (epc.veg_type == TREE) ){
		epv->day_livestem_turnover = (cs->live_stemc   + cs->livestemc_transfer  + cs->livestemc_store)
									* epc.livewood_turnover / 365;

		epv->day_livecroot_turnover = (cs->live_crootc + cs->livecrootc_transfer + cs->livecrootc_store)
									* epc.livewood_turnover	/ 365;
	}
	if (epc.veg_type == GRASS)
		epv->day_deadleaf_turnover = epc.deadleaf_turnover * (cs->dead_leafc ) / 365;
